              ...
              foreach ($orderItemList as $record) : ?>
                <tr><!-- Order Item -->
                  ...
                  <td style="border-left:double">
                    <input type="date" name="ordItems[<?= $record["OrderItemID"]; ?>][shippedDate]" value=<?= $record["ShippedDate"]; ?> /><?= " by " . $record["ShippedUserID"]; ?><br /><?= date("d/m/Y @ H:i", strtotime($record["EditTimestamp"])) . " by " . $record["EditUserID"];
                  ?></td>
                  <td>
